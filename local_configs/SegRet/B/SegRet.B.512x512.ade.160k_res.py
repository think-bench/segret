...
model = dict(
    ...
    decode_head=dict(
        ...
        dropout_ratio=0.1,
        num_classes=150,
        norm_cfg=norm_cfg, # dict(type='GN', num_groups=32),  # todo:the original is SyncBN, need to test which one is the best
        align_corners=False,
        decoder_params=dict(embed_dim=768, use_relu=True),
