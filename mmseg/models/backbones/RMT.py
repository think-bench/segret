...
class RetBlock(nn.Module):
    ...
    def __init__(self, retention: str, embed_dim: int, num_heads: int, ffn_dim: int, drop_path=0., layerscale=False,
                 layer_init_values=1e-5, norm_layer=None):
        super().__init__()
        ...
        if norm_layer is None:
            ...
            norm_layer['eps'] = 1e-6
            self.retention_layer_norm = build_norm_layer(norm_layer, self.embed_dim)[1]
        ...
        if norm_layer is None:
            ...
            norm_layer['eps'] = 1e-6
            self.final_layer_norm = build_norm_layer(norm_layer, self.embed_dim)[1]
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, out_dim, norm_layer=None):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Conv2d(dim, out_dim, 3, 2, 1)
        if norm_layer is not None:
            self.norm = build_norm_layer(norm_layer, out_dim)[1]
        else:
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, embed_dim, out_dim, depth, num_heads,
                 ...
                 ffn_dim=96, drop_path=0., norm_layer=None, chunkwise_recurrent=False,
...
def _get_norm_layer(norm_layer, num_features):
    if norm_layer is None:
        return nn.BatchNorm2d(num_features)
    else:
        return build_norm_layer(norm_layer, num_features)[1]
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.Sequential(
            ...
            _get_norm_layer(norm_layer, embed_dim // 2),
            nn.GELU(),
            ...
            _get_norm_layer(norm_layer, embed_dim),
            nn.GELU(),
    ...
    def forward(self, x):
        # B, C, H, W = x.shape # 原始图像大小
        x = self.proj(x).permute(0, 2, 3, 1)  # (b h w c)
        B, H, W, C = x.shape  # 经过卷积后的大小
        return x
...
class VisRetNet(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]  # 2 3 512 512 B C H W
        outs = []
        # x: B H W C
        x = self.patch_embed(x)  # 2 128 128 64 B H W C
        for layer in self.layers:
            x, x_no_conv, H, W = layer(x)  # x_no_conv 2 128 128 64 B H W C
            x_no_conv = x_no_conv.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            outs.append(x_no_conv)
...
class RMT_T(VisRetNet):
    def __init__(self, **kwargs):
        super(RMT_T, self).__init__(
...
@BACKBONES.register_module()
class RMT_S(VisRetNet):
    def __init__(self, **kwargs):
        super(RMT_S, self).__init__(
            embed_dims=[64, 128, 256, 512],
            depths=[3, 4, 18, 4],
            num_heads=[4, 4, 8, 16],
            init_values=[2, 2, 2, 2],
...
@BACKBONES.register_module()
class RMT_B(VisRetNet):
    def __init__(self, **kwargs):
        super(RMT_B, self).__init__(
...
@BACKBONES.register_module()
class RMT_L(VisRetNet):
    def __init__(self, **kwargs):
        super(RMT_L, self).__init__(
